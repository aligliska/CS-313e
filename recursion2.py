...
#  Description: This program uses recursion for different cases
#  for calculating if a list of numbers can make the targeted sum
#  or if two groups meet specified conditions. 
...
#  Date Created: 7/01/2021

#  Date Last Modified: 7/02/2021
...
def groupSum(start, nums, target):
    # base case if start is out of index
    if start >= len(nums):
        return target == 0 
    else:
        # either uses current value or rejects current value
...
def groupSum6(start, nums, target):
    ...
    if start >= len(nums):
        return target == 0
    # uses the current value '6' towards target sum
    elif nums[start] == 6:
        return groupSum6(start + 1, nums, target - nums[start])
    # if current value is not equal to 6
    else:
        # either uses current value or rejects current value
...
def groupNoAdj(start, nums, target):
    ...
    # use current value and skip next, or rejects current value
...
def groupSum5(start, nums, target):
    ...
    if start >= len(nums):
        return target == 0
    # uses the current value if multiples of 5 towards target sum
    elif nums[start] % 5 == 0 and (start < len(nums) -1):
        # skips next value if equal to 1
        if nums[start+1] == 1:
            return groupSum5(start + 2, nums, target - nums[start])
        # uses next value if not equal to 1
        else:
            return groupSum5(start + 1, nums, target - nums[start])
    # current value not a multiple of 5
    else:
        # either uses current value or rejects current value
...
def groupSumClump(start, nums, target):
    ...
    if start >= len(nums):
        ...
        # loop to find all adjacent duplicate values
        count = 1 
        while start < len(nums) - 1 and nums[start] == nums[start + count]: 
            count += 1
        # determines if value or lump of values are used for target sum
        return groupSumClump(start+count, nums, target - count*nums[start]) \
            or groupSumClump(start + count, nums, target)
...
# (No loops needed.)
def splitArray(nums):
    start, group1, group2 = 0, 0, 0
    return splitArrayHelper(start, nums, group1, group2)

def splitArrayHelper(start, nums, group1, group2):
    # base case if start is out of index
    if start >= len(nums):
        return group1 == group2
    # adds current value to group 1 or group 2
    else:
        return splitArrayHelper(start+1, nums, group1 + nums[start], group2) \
            or splitArrayHelper(start+1, nums, group1, group2 + nums[start])
...
def splitOdd10(nums):
    start, group1, group2 = 0, 0, 0
    return splitOdd10Helper(start, nums, group1, group2)
  
def splitOdd10Helper(start, nums, group1, group2):
    # base case if start is out of index
    if start >= len(nums):
        return (group1 % 10 == 0 and group2 % 2 != 0) \
            or (group2 % 10 == 0 and group1 % 2 != 0)
    # adds current value to group 1 or group 2
    else:
        return splitOdd10Helper(start+1, nums, group1 + nums[start], group2) \
            or splitOdd10Helper(start+1, nums, group1, group2 + nums[start])
...
def split53(nums):
    start, group1, group2 = 0, 0, 0
    return split53Helper(start, nums, group1, group2)

def split53Helper(start, nums, group1, group2):
    # base case if start is out of index
    if start >= len(nums):
        return group1 == group2 
    # adds multiples of 5 into group 1
    elif nums[start] % 5 == 0:
        return split53Helper(start + 1, nums, group1 + nums[start], group2)
    # adds multiples of 3 into group 2
    elif nums[start] % 3 == 0:
        return split53Helper(start + 1, nums, group1, group2 + nums[start])
    # distributes non-multiples of 3 and 5 into both groups 
    else:
        return split53Helper(start + 1, nums, group1 + nums[start], group2) \
            or split53Helper(start + 1, nums, group1, group2 + nums[start])
